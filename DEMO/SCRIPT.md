...
- **城市基本資訊**：查詢全球城市的基本資料，如人口、地理位置、時區等
- **即時天氣資訊**：獲取指定城市的當前天氣和未來預報
- **當地新聞整合**：查看與城市相關的最新新聞報導
- **多資訊整合**：綜合分析多種資訊，提供更完整的城市概況
...
- 程式碼必須包含適當的註解。
  - 所有的 Class 與 Method 一定要有註解
- 使用 Semantic Kernel 1.42.0 版本的 OpenAPI plugin
- 使用 Azure AI foundry 裡的 LLM API
- Semantic Kernel 啟用自動函式呼叫並使用自動函式呼叫
- 使用的 API 請尋找不需要 API KEY 的 OpenAPI

先不要執行，請先產出一個執行計畫與檔案文件目錄結構給我看，輸出請使用繁體中文，並列出預計要使用那些 OpenAPI
```